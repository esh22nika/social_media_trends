...
export function InteractiveNode({ data, isHovered, onHover, onLeave, onClick }: InteractiveNodeProps) {
  ...
  return (
    ...
    >
      ...
      {/* Main node container */}
      <div className="relative bg-white rounded-2xl shadow-2xl border-2 p-4 min-w-[200px]" style={{ borderColor: data.color }}>
        {/* Platform header */}
        <div className="flex items-center gap-2 mb-3 pb-2 border-b" style={{ borderColor: data.color + "40" }}>
          ...
          <div className="flex-1">
            <div className="uppercase tracking-wider opacity-60" style={{ fontSize: "10px" }}>
              {data.platform}
            </div>
            <div className="truncate max-w-[120px]">{data.topic}</div>
          </div>
        </div>
